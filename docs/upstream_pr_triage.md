...
- **2026-08-31** — **Minimum detectable effect** (from #162's statistics, rebuilt) **added**. A gate
  ...
  **lower bound**: these use the normal approximation while the correct quantile at n of 8-20 comes
  ...
  Diagnostics only — nothing reads them. That is testable rather than asserted: the decision
  payload's key structure was snapshotted **before** this work and committed, and a test compares
  against it, so a diagnostic leaking into a verdict fails. Exact paired tests (sign-flip, McNemar)
  stay deferred rather than shipped with unpinned conventions. 22 tests, both functions checked
  against independently hand-computed values; full non-slow suite green (1835 passed, +22), ruff
  clean.
...
| #162 (partial), #136 | **Minimum detectable effect** as a gate-adjacent diagnostic | A gate can certify a win, or enforce a regression floor, without ever stating that its sample size could not detect the effect it claims to police. Absorbs the parked exact-test item. | `evolution/core/stats.py` (currently `paired_bootstrap` only) | **DONE** — `min_detectable_effect_paired` + `min_detectable_shift_paired_binary` in `stats.py`, surfaced as `power_diagnostics.json` beside the decision; α and sidedness derived from the bootstrap's own confidence, figures labelled lower bounds; invariance proven against a decision-payload golden captured before the work — see review log, 2026-08-31 | ✅ |
