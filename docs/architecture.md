...
The tier supports **two agent backends** behind the same three protocols, selected once by the `build_backend` factory (`evolution/prompts/backend.py`) into a `PromptBackend` strategy — `evolve_prompt_section` itself has no per-target branches. Selected by `--target`:
