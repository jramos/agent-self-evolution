...
- `evolution/validation/report.py` — `_score_convention` + the `expected_action == "convention"` branch in `score_task` (agent-agnostic). Convention-adherence verdict, no LLM judge: pass iff some `Bash` call's command contains one of `required_cmd_substr` (the repo wrapper) AND no `Bash` command contains any of `forbidden_cmd_substr` (the default tool to avoid). Reads only `tool_calls_with_args`, so it is backend-independent.
