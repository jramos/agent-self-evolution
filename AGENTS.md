...
- **`generate_findings_report.py` renders the cross-run synthesis reports** (e.g. `reports/asymmetry_report.pdf`, `reports/phase4_validation_report.pdf`, `reports/phase5_validation_report.pdf`) from a self-contained `reports/<name>_prose.yaml` — no run dir. It reuses `generate_report.py`'s shared title/footer/table/style primitives, and verifies every number in the prose's `provenance` block against its source JSON (a check may carry its own `source` for multi-source synthesis), failing closed if any number drifts: `python generate_findings_report.py --prose reports/<name>_prose.yaml --out reports/<name>.pdf`.
